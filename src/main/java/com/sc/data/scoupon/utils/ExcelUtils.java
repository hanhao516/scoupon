...
public class ExcelUtils {
     ...
     * @throws Exception
     */
    public static List<Map<String, Object>> readExcelReMap(File file) throws Exception {
        if(!file.exists()){
        ...
        for (int i = sheet.getFirstRowNum(); i <= (sheet.getPhysicalNumberOfRows() - 1); i++) {
            ...
            for (int j = row.getFirstCellNum(); j <= row.getLastCellNum(); j++) {
                ...
                if (value != null && !value.equals("")&&!value.equals(" ")) {
                    //单元格不为空，则加入列表
                	if(f==0){
                		fieldName.add((String) value);
                	}else{
                		map.put(fieldName.get(index), value);
                	}
    ...
    public static void createExcel(String excel_name, String[] headList,
        ...
        XSSFCellStyle style_blue = workbook.createCellStyle();
        ...
        style_blue.setFillPattern(XSSFCellStyle.FINE_DOTS );
        style_blue.setFillForegroundColor(IndexedColors.BLUE.getIndex());
        style_red.setFillPattern(XSSFCellStyle.FINE_DOTS );
        style_red.setFillForegroundColor(IndexedColors.RED.getIndex());
        style_yellow.setFillPattern(XSSFCellStyle.FINE_DOTS );
        ...
        for (int n = 0; n < dataList.size(); n++) {
            ...
            for (int i = 0; i < fieldList.length; i++) {
                ...
                // 在单元格中输入一些内容
                String cellVal = dataMap.get(fieldList[i]) == null ? "":(dataMap.get(fieldList[i])).toString();
